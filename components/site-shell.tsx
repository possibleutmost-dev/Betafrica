...
export function SiteShell({ children }: { children: ReactNode }) {
  ...
  if (bare) {
    return <ShellContext.Provider value={shell}><main className="min-h-screen bg-[#eef0f4] text-[#132640]">{children}</main></ShellContext.Provider>
  }
      ...
      <main className="min-h-screen bg-[#f3f6f9] text-[#0f172a]">
        <header className="sticky top-0 z-40 bg-[#0b1b33] text-white">
          <div className="mx-auto flex max-w-[1180px] items-center gap-2 px-3 py-2.5 sm:gap-4 sm:px-4 sm:py-3">
            ...
            <Link href="/" className="flex items-center gap-1.5 whitespace-nowrap text-[20px] font-extrabold tracking-tight sm:text-[24px]"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#facc15] text-[15px] font-black text-[#0b1b33]">W</span>Winn<span className="-ml-1.5 text-[#facc15]">Bet</span></Link>
            ...
            <div className="ml-auto flex min-w-0 items-center gap-1 sm:gap-2">
              {player ? (
                <>
                  ...
                  <Link href="/deposit" className="flex h-8 shrink-0 items-center rounded-full bg-[#facc15] px-3.5 text-xs font-bold text-[#0f172a] sm:h-9 sm:px-4 sm:text-sm">Deposit</Link>
                  <button onClick={() => signOut()} className="hidden h-9 px-3 text-sm font-semibold sm:block">Logout</button>
                </>
              ) : (
                <>
                  <button onClick={() => setAuth('register')} className="h-8 rounded-full bg-[#facc15] px-3.5 text-xs font-bold text-[#0f172a] sm:h-9 sm:px-5 sm:text-sm">Join Now</button>
        ...
        {!gamesPage && !accountPage && (
          <div className="overflow-hidden border-b border-[#e2e8f0] bg-white">
            <div className="scrollbar-none -mb-5 mx-auto flex max-w-[1180px] overflow-x-auto px-1 pb-5 sm:px-4">
              {sportTabs.map(([label, href]) => (
                <Link key={label} href={href} className={`whitespace-nowrap border-b-[3px] px-3 py-3 text-[13px] sm:px-4 sm:text-sm ${isActive(href) ? 'border-[#0d9488] font-semibold text-[#0f766e]' : 'border-transparent text-[#64748b]'}`}>{label}</Link>
              ))}
            </div>
          </div>
        )}
        ...
        {notice && (
          <div className={`fixed left-1/2 z-40 flex w-max max-w-[calc(100vw-2rem)] -translate-x-1/2 items-center rounded-xl bg-[#0f172a] px-5 py-3 text-sm text-white shadow-xl bottom-40 md:bottom-6`}>
            {notice}
        ...
        {!gamesPage && !accountPage && (
          <button
            ...
            className="fixed bottom-[84px] right-4 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-[#0d9488] text-white shadow-[0_6px_20px_rgba(13,148,136,0.4)] md:hidden"
            aria-label={`Open bet slip, ${slipCount} selections`}
          >
            <ReceiptText size={24} />
            <span className="absolute -right-0.5 -top-0.5 flex h-6 min-w-6 items-center justify-center rounded-full border-2 border-white bg-[#facc15] px-1 text-[11px] font-extrabold text-[#0f172a]">{slipCount}</span>
          </button>
        )}
        ...
        <nav className="fixed inset-x-0 bottom-0 z-30 grid h-16 grid-cols-5 border-t border-[#e2e8f0] bg-white pb-[env(safe-area-inset-bottom)] text-[11px] text-[#94a3b8] md:hidden" aria-label="Main">
...
function TabLink({ href, active, icon, label, badge = 0 }: { href: string; active: boolean; icon: ReactNode; label: string; badge?: number }) {
  return (
    <Link href={href} className={`relative flex flex-col items-center justify-center gap-1 ${active ? 'font-semibold text-[#0f766e] shadow-[inset_0_2px_0_#0d9488]' : ''}`}>
      {icon}
      {label}
