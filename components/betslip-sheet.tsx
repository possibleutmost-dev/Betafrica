...
export default function BetslipSheet({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:hidden" role="dialog" aria-modal="true" aria-label="Bet slip">
      <button className="absolute inset-0 bg-[#14162e]/55" onClick={onClose} aria-label="Close bet slip" />
      <div className="relative max-h-[85vh] w-full overflow-y-auto rounded-t-3xl bg-[#eef0f6] p-3 pb-[max(12px,env(safe-area-inset-bottom))]">
        <div className="mx-auto mb-2 h-1.5 w-10 rounded-full bg-[#c9cde0]" />
        <button onClick={onClose} className="absolute right-4 top-4 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white text-[#6b7087] shadow-sm" aria-label="Close bet slip"><X size={18} /></button>
