...
function ModalHeader({ title, onClose }: { title: string; onClose: () => void }) {
  return (
    <div className="flex items-center justify-between bg-[#ed1324] px-4 py-3 text-white">
      <span className="text-lg font-black italic tracking-[-1px]">SportyBet</span>
...
export function BookedCode({ code, expiresAt, onClose }: { code: string; expiresAt?: string | null; onClose: () => void }) {
  ...
  const message = `Load my SportyBet code ${code} — ${link}`
  ...
  const shareInApp = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: `SportyBet code ${code}`, text: message, url: link })
        return
      } catch {
        return
      }
    ...
    <Modal onClose={onClose}>
      ...
      <div className="px-5 pb-5 pt-4 text-[#24262c]">
        ...
        <div className="mt-4 grid grid-cols-5 gap-1">
          ...
          <ShareAction label="Save" href={imageUrl} download={`sportybet-${code}.png`} icon={<Download size={18} />} />
        </div>
...
export function WinCelebration({ code, amount, currency, onClose }: { code: string; amount: number; currency: string; onClose: () => void }) {
  ...
  const share = async () => {
    const text = `I just won ${formatMoney(amount, currency)} on SportyBet. Ticket ${code}.`
    const url = window.location.origin
    if (navigator.share) {
      try {
        await navigator.share({ title: 'SportyBet win', text, url })
        return
      } catch {
        return
      }
