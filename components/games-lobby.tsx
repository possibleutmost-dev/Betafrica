...
const BADGE_STYLE = {
  POPULAR: 'bg-[#ed1324]',
  NEW: 'bg-[#0d9488]',
  EXCLUSIVE: 'bg-[#1f1f1f] ring-1 ring-[#facc15] text-[#facc15]',
}
  ...
  return (
    <div className="games-felt min-h-[calc(100vh-120px)] py-5">
      <div className="mx-auto grid max-w-[1180px] grid-cols-1 gap-3 px-3 sm:px-4 md:grid-cols-[180px_minmax(0,1fr)]">
        <aside className="min-w-0 self-start overflow-hidden bg-[#24395a] text-white">
          <nav className="scrollbar-none -mb-5 flex overflow-x-auto pb-5 md:mb-0 md:flex-col md:pb-0">
            {CATEGORIES.map(({ key, label, icon: Icon }) => (
              <button
                key={key}
                onClick={() => setCategory(key)}
                className={`flex shrink-0 items-center gap-3 px-3 py-3 text-left text-xs font-semibold ${category === key ? 'bg-[#0d9488]' : 'hover:bg-white/10'}`}
              >
                <Icon size={15} /> {label}
              </button>
            ))}
        ...
        <div className="min-w-0 space-y-3">
              ...
              <div className="scrollbar-none -mb-5 flex gap-3 overflow-x-auto pb-5">
                {wins.map((win) => (
                  <div key={win.code} className="flex shrink-0 items-center gap-3 border border-white/20 bg-black/40 px-3 py-2">
                    <TrophyIcon size={22} className="text-[#facc15]" />
                    <div>
                      <p className="text-sm font-black text-[#facc15]">{formatMoney(win.amount, win.currency)}</p>
          ...
          <div className="bg-[#24395a] p-3 sm:p-4">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <h1 className="text-lg font-bold text-white">{title}</h1>
              <label className="flex h-9 w-full items-center gap-2 bg-[#1b2f4d] px-3 text-sm text-white/70 sm:w-[240px]">
                <Search size={15} />
  ...
  return (
    <Link href={`/games/${game.slug}`} className="group relative block aspect-square overflow-hidden rounded-lg text-left">
      ...
      <span className="absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 transition-opacity group-hover:opacity-100">
        <span className="bg-[#0d9488] px-5 py-2 text-sm font-bold text-white">PLAY</span>
      </span>
    </Link>
  )
}
