  ...
  return (
    <section className="mx-auto max-w-[900px] px-3 py-6 sm:px-4 sm:py-12">
      <div className="bg-white p-5 shadow-sm sm:p-8">
        <h1 className="text-2xl font-black sm:text-3xl">{title === 'Help' ? 'How BetAfrica works' : title}</h1>
        <p className="mt-3 text-[#64748b]">Create an account, fund your wallet, choose a fixture or game, and review your selections before placing a bet.</p>
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {['Create your account', 'Choose your market', 'Withdraw winnings'].map((step, i) => (
            <div key={step} className="border p-5">
              <span className="text-sm font-bold text-[#0f766e]">0{i + 1}</span>
              <h2 className="mt-3 font-bold">{step}</h2>
              <p className="mt-2 text-sm leading-6 text-[#64748b]">Review the applicable rules before continuing.</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section className="mx-auto max-w-[480px] px-4 py-10">
      <div className="bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-[#64748b]">Enter a code someone shared with you to get the same selections on your betslip.</p>
        <form onSubmit={(event) => { event.preventDefault(); submit() }} className="mt-5 space-y-3">
          <input
            ...
            placeholder="ABC123"
            maxLength={8}
            className="h-14 w-full border text-center text-2xl font-black tracking-[0.25em] outline-none focus:border-[#0d9488]"
          />
          {error && (
            <p className="bg-[#fff0f1] px-3 py-2 text-xs text-[#0f766e]">
              {error}
              {ticket && <> <Link href={`/my-bets/${ticket}`} className="font-bold underline">Open it in My Bets</Link></>}
            </p>
          )}
          <button type="submit" disabled={busy || code.trim().length < 4} className="h-11 w-full rounded-xl bg-[#facc15] text-sm font-bold text-[#0f172a] disabled:opacity-50">{busy ? 'Loading…' : 'Load code'}</button>
        </form>
      </div>
    </section>
  )
